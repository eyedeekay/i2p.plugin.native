...
clientApp.0.name=karen
clientApp.0.args=chmod +x $PLUGIN/lib/karen
clientApp.0.delay=0
...
func (cc *ClientConfig) Print() string {
	r := "clientApp.1.main=net.i2p.util.ShellCommand\n"
...
func (cc *ClientConfig) PrintCommand() string {
	if *targetos == "windows" {
		if cc.Command == nil || *cc.Command == "" {
			if cc.ClientName != nil || *cc.ClientName != "" {
				return fmt.Sprintf("clientApp.1.args=$PLUGIN/lib/karen.exe -exeperm \"0755\" -exe %s -args \"%s\" -exedir $PLUGIN/lib/ -instruct start\n", *cc.ClientName, cc.PrintCommandArgs())
			}
			log.Fatal("-name is a required field.")
		}
		...
		if cc.Command == nil || *cc.Command == "" {
			if cc.ClientName != nil || *cc.ClientName != "" {
				return fmt.Sprintf("clientApp.1.args=$PLUGIN/lib/karen -exeperm \"0755\" -exe %s -args \"%s\" -exedir $PLUGIN/lib/ -instruct start\n", *cc.ClientName, cc.PrintCommandArgs())
			}
			log.Fatal("-name is a required field.")
		}
		return fmt.Sprintf("clientApp.1.args=$PLUGIN/lib/karen -exeperm \"0755\" -exe %s -args \"%s\" -exedir $PLUGIN/lib/ -instruct start\n", strings.Split(*cc.Command, " ")[0], cc.PrintCommandArgs())
...
func (cc *ClientConfig) PrintStart() string {
	...
	return fmt.Sprintf("clientApp.1.startOnLoad=%t\n", *cc.Start)
}
